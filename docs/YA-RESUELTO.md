...
- **Orden 2: Redes sociales, WhatsApp personal, agenda y modelos de portada (22 de agosto de 2026):**
  ...
  - **Bloque 2.3 — Agenda y alertas al celular (`src/app/actions/notifications.ts`, `src/lib/automatico/al-entrar-a-la-app.ts`, `src/lib/automatico/tareas-automaticas.ts`, `src/app/actions/agenda.ts`):** Alerta prioritaria de reunión en 1 hora ejecutada de forma desatendida por el despachador unificado (`reuniones-agenda`) con deduplicación por ventana de 18 horas. Unificación completa de reuniones de fiesta y citas en `getCalendarEvents()`.
  - **Bloque 2.4 — Modelos de portada visuales (`src/types/landing-editor.ts`, `src/app/(app)/empresa/landing-editor/page.tsx`, `src/__tests__/redes-sociales-transparencia-y-modelos.test.ts`):** Selector de 4 modelos de portada en criollo ("Elegante", "Fiesta", "Sobrio", "Moderno") con paletas y badges preconfigurados, vista previa en tiempo real y botón para volver al modelo anterior con un toque, manteniendo títulos, descripciones y posicionamiento SEO 100% intactos.
  - **Bloque 2.5 — Publicación y métricas en Google Business (`src/types/social-media.ts`, `src/lib/presencia-digital/publicador.ts`, `src/app/actions/seo-posicionamiento.ts`, `src/app/(app)/empresa/presencia-digital/seo-posicionamiento-tab.tsx`):** Integración de Google como red social en el planificador, validación de conexión real con explicación de vinculación, y visualización de la fecha de última publicación en el panel de posicionamiento SEO.
  - **Tanda 1.5 — Clasificación y auditoría de pantallas olvidadas (`docs/COBERTURA-AUDITORIA.md`):** Auditoría exhaustiva de las 9 pantallas olvidadas y 4 áreas de cobertura, certificando que las redirecciones de `/prospectos`, `/portal-cliente`, `/presentacion` e `/invitado` son seguras y que `/comparativa-ganancias`, `/configuracion/backup-final`, `/recepcion/:fiestaId` y `/portal-invitado` funcionan de forma aislada y consistente.
  - **Bloque 0 — Modelos Gemini y Capa de Inferencia (`src/ai/genkit.ts`, `src/__tests__/asistente-modelos-y-fallback.test.ts`):** Se configuró `googleai/gemini-flash-latest` como modelo por defecto para auto-actualización continua ante nuevas versiones de Google AI y `googleai/gemini-2.5-pro` fijado para razonamiento complejo. Conmutación automática a `geminiFastModel` ante saturación o al alcanzar el tope presupuestario mensual (`hayPresupuestoParaIA`).
  - **Bloque 1 — Extracción e ingesta asistida de prospectos (`src/lib/multiagent/assistant-crm-actions.ts`, `src/__tests__/asistente-crm-prospecto-confirmacion.test.ts`):** Extracción estructurada de prospectos a partir de lenguaje natural libre con verificación previa de duplicados por teléfono/nombre y requerimiento de confirmación explícita antes del guardado.
  - **Bloque 2 — Borrador de presupuestos vinculado a catálogo oficial (`src/lib/multiagent/assistant-crm-actions.ts`, `src/__tests__/asistente-presupuesto-catalogo.test.ts`):** Generación de borradores de cotización calculando precios exclusivamente desde el catálogo de servicios oficial de AK, sin invención de montos y aplicando guardrails financieros.
  - **Bloque 3 — Preparación de mensajes de WhatsApp (`src/lib/multiagent/assistant-crm-actions.ts`):** Redacción de mensajes de seguimiento comercial, aviso de cuota y saludo post-fiesta en formato uruguayo listos para despachar desde WhatsApp personal sin envíos autónomos.
  - **Bloque 4 — Diagnóstico matutino proactivo (`src/lib/automatico/repaso-matutino-ia.ts`, `src/app/(app)/repaso-diario/page.tsx`):** Diagnóstico diario cacheado que consolida cuotas por vencer en la semana, presupuestos fríos (>14 días), pendientes de la fiesta más próxima y alertas de tareas automáticas.
  - **Bloque 5 — Asistentes contextuales para portales (`src/app/actions/asistente-virtual.ts`, `src/__tests__/asistente-aislamiento-fiestas-y-seguridad.test.ts`):** Asistente para portal cliente aislado a su propia fiesta, y asistente para portal de invitados con bloqueo absoluto de datos financieros o de personas ajenas.
  - **Bloque 6 — Memoria y feedback (`src/app/actions/assistant.ts`, `src/types/multiagent.ts`):** Registro de feedback positivo/negativo y citas de fuentes en respuestas con datos numéricos.

- **Orden 3: La asistente de la app, al máximo (22 de agosto de 2026):**
  - **Bloque 0 — Modelos Gemini y Capa de Inferencia (`src/ai/genkit.ts`, `src/__tests__/asistente-modelos-y-fallback.test.ts`):** Se configuró `googleai/gemini-flash-latest` como modelo por defecto para auto-actualización continua ante nuevas versiones de Google AI y `googleai/gemini-2.5-pro` fijado para razonamiento complejo. Conmutación automática a `geminiFastModel` ante saturación o al alcanzar el tope presupuestario mensual (`hayPresupuestoParaIA`).
  - **Bloque 1 — Extracción e ingesta asistida de prospectos (`src/lib/multiagent/assistant-crm-actions.ts`, `src/__tests__/asistente-crm-prospecto-confirmacion.test.ts`):** Extracción estructurada de prospectos a partir de lenguaje natural libre con verificación previa de duplicados por teléfono/nombre y requerimiento de confirmación explícita antes del guardado.
  - **Bloque 2 — Borrador de presupuestos vinculado a catálogo oficial (`src/lib/multiagent/assistant-crm-actions.ts`, `src/__tests__/asistente-presupuesto-catalogo.test.ts`):** Generación de borradores de cotización calculando precios exclusivamente desde el catálogo de servicios oficial de AK, sin invención de montos y aplicando guardrails financieros.
  - **Bloque 3 — Preparación de mensajes de WhatsApp (`src/lib/multiagent/assistant-crm-actions.ts`):** Redacción de mensajes de seguimiento comercial, aviso de cuota y saludo post-fiesta en formato uruguayo listos para despachar desde WhatsApp personal sin envíos autónomos.
...
- **Orden 1: Automatización 100% autónoma, cuatro arreglos de auditoría, movimiento pulido, maquetación de control y despliegue del despertador (22 de agosto de 2026):**
  - **Bloque 1 — Que corra solo de verdad (`functions/src/index.ts`, `src/lib/automatico/control-concurrencia.ts`, `src/lib/automatico/al-entrar-a-la-app.ts`, `src/app/api/cron-despachador/route.ts`, `src/app/page.tsx`, `docs/PRENDER-LAS-TAREAS.md`, `src/__tests__/concurrencia-tareas-automaticas.test.ts`):** Se programó una Cloud Function única (`despertadorTareasAutomaticas`) cada 15 minutos en horario de Uruguay que llama al despachador unificado (`/api/cron-despachador`). Las visitas a la portada pública lanzan la puesta al día en segundo plano sin demoras. Candado sincrónico atómico y persistente (`intentarAdquirirLock`) que bloquea microtareas concurrentes antes del primer I/O, evitando dobles corridas y gasto de IA ante ráfagas. En `/settings/tareas-automaticas` se visualiza el origen exacto del disparo y las atrasadas en rojo.
  - **Bloque 2 — Cuatro arreglos chicos de auditoría general:**
    - *Lista de compras vacía (`src/app/(app)/fiestas/nueva/catering/lista-compras/page.tsx`):* Cuando un evento no tiene platos con ingredientes cargados, se muestra una tarjeta explicativa con botones de acceso directo a "Cargar Menú y Platos" y "Configurar Bebidas y Barra".
    - *Presupuestos impresos sin centavos (`src/components/budget/BudgetDocument.tsx`, `src/components/presupuestos/BudgetPrintTemplate.tsx`, `src/components/presupuestos/paso-4-resumen.tsx`):* Se unificó el formato de moneda en cero decimales (`minimumFractionDigits: 0, maximumFractionDigits: 0`) para coincidir con el resto de la app y no mostrar ",00" en presupuestos para el cliente.
    - *Botón de eliminar factura con pagos (`src/components/invoice-list-item.tsx`):* El botón de eliminar se muestra deshabilitado y atenuado cuando la factura tiene pagos registrados (`invoice.payments.length > 0` o `status === 'Paid'`) con el tooltip explicativo *"No se puede eliminar una factura con pagos registrados"*.
    - *Aviso en reacción de moodboard (`src/app/portal/[fiestaId]/moodboard/page.tsx`):* Si falla la comunicación al guardar el corazón/reacción del cliente, se muestra un mensaje de error descriptivo (*"No pudimos guardar tu reacción. Por favor probá de nuevo."*) antes de revertir.
  - **Bloque 3 — Remate del movimiento en pantallas públicas (`src/components/ui/animated-counter.tsx`, `src/components/landing/StatsSection.tsx`, `src/components/landing/GallerySection.tsx`, `src/components/landing/HeroSection.tsx`):** Contador animado que trepa desde 0 al entrar en pantalla respetando `useReducedMotion()`. Galería con entrada escalonada (`index % 12`) al tocar "Ver más" y molde gris esqueleto mientras cargan las fotos. Removidas dos animaciones continuas pesadas en portada para ahorrar batería en móviles.
  - **Bloque 4 — Huella de maquetación y estabilidad de pruebas E2E (`tests/e2e/layout-baseline.spec.ts`, `tests/e2e/layout-baseline.json`):** Se corrigió la persistencia de referencias faltantes cuando `UPDATE_MISSING_LAYOUT_BASELINE=true` se activa para perfiles individuales, y se incorporaron las referencias de `/presupuestos/nuevo` para `chromium-desktop` y `chromium-mobile`.
  - **Bloque 5 — Publicación y monitoreo del despertador de Google (`firebase.json`, `.github/workflows/deploy.yml`, `src/lib/automatico/tareas-automaticas.ts`, `src/app/api/cron-despachador/route.ts`, `src/app/actions/tareas-automaticas.actions.ts`, `src/app/(app)/settings/tareas-automaticas/page.tsx`, `src/__tests__/despliegue-incluye-despertador.test.ts`):** Se configuró la sección `functions` en `firebase.json` y se integró el despliegue automático de Cloud Functions en `.github/workflows/deploy.yml` mediante `firebase-tools deploy --only functions`. Se implementó el registro de toques en puerta (`marcarToqueDespertador`) y la consulta de estado (`estadoDelDespertador` / `getEstadoDespertadorAction`), mostrando en `/settings/tareas-automaticas` cuándo tocó la puerta y alertando en rojo si nunca tocó o está atrasado. Se crearon pruebas unitarias automatizadas para asegurar que el despliegue siempre incluya las funciones.
...
- **Auditoría de metadatos SEO reales y sin jerga técnica (22 de agosto de 2026):**
  ...
  - **Remoción de jerga técnica para el dueño (`src/app/actions/conexiones-estado.actions.ts`, `src/app/(app)/settings/sincronizaciones/page.tsx`):** Se eliminaron los nombres de variables de entorno (`INSTAGRAM_ACCESS_TOKEN`, `INSTAGRAM_BUSINESS_ACCOUNT_ID`) y permisos técnicos (`instagram_basic`) de los mensajes y de la guía paso a paso, explicando en criollo qué datos faltan y dónde se cargan.
...
## 16 de septiembre de 2026 - Continuacion Codex: respaldos

Main da6c566fbefdf82efec484f7fc9830db6ab5c4de. Contraste de cinco PR abiertas: ninguna modifica estos archivos. Evidencia en docs/auditoria/RESPALDOS-2026-09-16.md y sonda docs/evidencias/backup-fallos-reales.cjs. 7 casos aislados: 4 PASS y 3 FAIL. BKP01: lectura parcial produce manifiesto complete/success=true. BKP02: deleteRestorePoint acepta sesion de operador sin permiso de administracion. BKP03: restauracion parcial HTTP200 muestra Completa y recarga. Observacion estatica adicional: backupStatus=null presenta ACTIVO Y PROTEGIDO, pendiente prueba visual. Codex no modifico app ni datos; no restauro/borro copias reales ni compilo. Destino Claude por permisos/datos; observaciones para correccion despues del diagnostico, no tarea iniciada automaticamente. Pruebas de sesion/Firebase reales y restauracion integral siguen pendientes. No reabrir como duplicado el fallo antiguo de fecha de backup: el nuevo caso devuelve exito pese a faltar una lectura.
...
## 2026-09-22 - Invitados, barra y catalogos: ocho hallazgos

Base main54cd623. Informe docs/auditoria/BARRA-INVITADOS-CATALOGOS-2026-09-22.md y orden78 en docs/ordenes/78-invitados-barra-y-catalogos-sin-perdidas.md.20 sondas aisladas:9 PASS/11 FAIL. INV01 respuesta RSVP incluye updatedFiesta/otros invitados; INV02 nombre coincidente devuelve credencial, sin quitar decision de poder cambiar RSVP. BAR01 alta fallida descuenta stock y puede anunciar exito; BAR02 cola fallback no se recupera tras error; BAR03 reemplazo fallido cancela pedido previo. CAT01 altas de fotos simultaneas pierden una; CAT02 editar salones distintos revierte uno; CAT03 ultimo borrado ignorado por proteccion de array vacio. Persistencia contrastada con syncToFirestore, no emulador/produccion. Las4PR abiertas no modifican archivos afectados. Claude:datos/stock/permisos/compilacion; Gemini:UI. Agente economico propuso candidatos de catalogos, Codex reprodujo; cerrado. No app modificada ni fusion. NO cierre total: quedan pruebas reales por roles, hardware, terceros y compilacion integrada. Tests de orden78 propuestos, no ejecutados ni declarados existentes.
...
## 2026-09-23 - Codex: orden81, cierre por recorridos y evolucion sin duplicar

Solo documentacion, no implementacion. Dueno autorizo preparar trabajo para Gemini/Claude. Main2f41322 y HEAD de cinco PR abiertas inventariados en docs/ordenes/81-cierre-recorridos-y-evolucion.md. Se verifico que YA existen colas/capturas offline, galeria y Centro de Fiesta (show-control redirige): no reconstruirlos. Se entrega matriz docs/auditoria/81-matriz-recorridos.md, pendiente de reconciliar evidencia previa. Incluye roles, venta/PDF/CRM, dinero, planificacion, portales, entretenimiento, conexiones, visual/velocidad y hardware. Nuevas mejoras: completar continuidad existente, entrega sin ocupar camara, interaccion coordinada y comprobacion previa dentro del centro; no nuevas plataformas/suscripciones. Codex revisa; Gemini interfaz/entretenimiento; Claude dinero/comida/permisos/datos y compilacion. Orden80 residuales y devolucion1214 no se duplican. No pruebas nuevas de app ni build ni fusion en esta entrega. No certificado de lanzamiento.
