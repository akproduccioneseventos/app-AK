...
- **La estética se revisó también en las pantallas del EQUIPO, no sólo en las públicas (15 de agosto de 2026).** El dueño lo pidió así: *"estética de toda la app, no sólo la web"*. Se sacaron fotos de las pantallas internas con sesión iniciada (`AK_FOTOS=true`, bloque "fotos de las pantallas del equipo" en `tests/e2e/fotos-de-la-app.spec.ts`) y se miraron a ojo. Lo encontrado quedó repartido así: **el aviso del planificador que mostraba los asteriscos de negrita crudos (`**no elimina**`) lo arregló Claude**, porque es la pantalla de comida; **los dos problemas de botones flotantes fueron a la orden de Gemini**, porque son de pantallas.
- **En el celular, el botón flotante de volver se monta sobre el título de TODAS las pantallas internas.** `module-navigation-dock.tsx` está clavado en `top-20 left-3` encima del contenido. En Pagos Rápidos tapa media frase, justo en la pantalla que dice ser para usar desde el celular. **Está pedido a Gemini, no lo arregles por tu cuenta si ves la orden abierta.**
