...
- **El panel de presencia digital que el dueño pueda usar solo (18 de agosto de 2026).**
  - **Bloque 1 (Que lo programado salga de verdad):** Implementado `procesarPosteosProgramados()` en `src/lib/presencia-digital/programador-posteos.ts` y conectado a las tareas cron (`metricas-de-redes/route.ts` y `publicaciones-programadas/route.ts`). Aplica tope estricto de máximo 3 publicaciones por corrida para evitar spam tras caídas, límite de 3 reintentos con registro de error antes de marcar `Error`, y pasa redes no automatizables (TikTok, WhatsApp, Threads, X) a estado `Listo para copiar` avisando que requieren publicación manual.
  - **Bloque 2 (Textos de marketing con IA sin repeticiones):** En `src/app/actions/social-media.ts`, `generateDraftsFromFiesta` ahora llama al agente de marketing con IA (`chatWithMarketingAgent`) usando los datos reales de la fiesta (tipo, salón, invitados, dedicatorias de fotos) y pasa obligatoriamente por el contador de gasto (`hayPresupuestoParaIA` y `registrarConsumoIA('generador-textos-marketing')`). Si no hay presupuesto o falla la IA, cae limpiamente al fallback de plantillas sin romper la pantalla.
  - **Bloque 3 (Qué red le trae clientes de verdad):** Nuevo motor de atribución `calcularAtribucionComercialPorRed()` en `src/lib/presencia-digital/atribucion-redes.ts` y pestaña *"Clientes por Red"* en `/empresa/presencia-digital`. Cruza prospectos con presupuestos reales aceptados/facturados y muestra consultas, contratos y dinero por canal en períodos de 30, 90 y 365 días. Cero números inventados: si de una red no vino nadie, muestra 0 y el mensaje claro (*"De [Red] no vino ninguna consulta en estos [X] días"*).
  - **Bloque 4 (Aviso visible de inactividad):** Banner superior en el centro de presencia digital que alerta visiblemente si hace más de 3 días que no se publica en alguna red con botón directo para crear un posteo con la última fiesta, o felicita al dueño con mensaje verde si viene publicando parejo.
  - **Validación:** Suite dedicada `panel-presencia-digital-usable.test.ts` con 5 de 5 pruebas pasando, typecheck limpio, control de acentos en verde y build de Next.js sin errores en las 274 páginas.
