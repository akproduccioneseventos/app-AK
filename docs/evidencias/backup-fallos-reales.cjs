...
async function backup(failRead) {
  let manifest;
...
async function restoreUi(partial) {
  ...
  await evaluate(extract('backup-page.tsx',['handleRestoreFromZip']),scope,'handleRestoreFromZip()');
...
(async()=>{
  const results=[await backup(false),await backup(true),await deletion(false,'user'),await deletion(true,'admin'),await deletion(true,'user'),await restoreUi(false),await restoreUi(true)];
