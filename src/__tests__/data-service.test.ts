...
jest.mock("fs/promises", () => ({
  ...
  readFile: jest
    .fn()
    .mockRejectedValue(
      new Error("Filesystem reads are disabled in this unit test."),
    ),
}));
...
describe("data-service — writeData validation", () => {
  ...
  it("rejects path traversal attempts", async () => {
    ...
    await expect(writeData("../etc/passwd", "malicious")).rejects.toThrow(
  ...
  it("blocks partial updates while browser tests use local JSON data", async () => {
    ...
    const { updateDataPartial } = await import("@/lib/data-service");
    await expect(
      updateDataPartial("test.json", { status: "changed" }),
    ).rejects.toThrow(
      "La escritura esta deshabilitada en el modo local de pruebas.",
    );
  });
  ...
  it("can skip automatic backup during a restore", async () => {
    ...
    const { writeData } = await import("@/lib/data-service");
    await writeData("customers.json", [{ id: "1" }], undefined, {
      skipAutoBackup: true,
    });
