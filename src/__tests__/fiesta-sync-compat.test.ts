import {
  applyLaundryCosts,
  deriveBudgetModulesForSync,
  mergeClientPortalSettingsForSync,
  replaceAutoGeneratedCostItems,
} from '@/lib/fiesta-sync-utils';
...
describe('syncFiestaFromBudget compatibility helpers', () => {
  ...
  it('reemplaza costos automáticos sin duplicarlos y conserva los manuales', () => {
    const current = [
      { id: 'manual-1', nombre: 'Flete', category: 'Manual', montoEstimado: 800 },
      { id: 'auto_amort_tech', nombre: 'Amortización anterior', category: 'Gasto Fijo', montoEstimado: 200 },
      { id: 'auto_merma_bebidas', nombre: 'Merma anterior', category: 'Insumos', montoEstimado: 50 },
    ];
    const generated = [
      { id: 'auto_amort_tech', nombre: 'Amortización actual', category: 'Gasto Fijo', montoEstimado: 250 },
    ];

    expect(replaceAutoGeneratedCostItems(current, generated)).toEqual([
      current[0],
      generated[0],
    ]);
  });

  it('no muta el costo de lavadero recibido al recalcularlo', () => {
    const original = [{ id: 'laundry', nombre: 'Lavado Mantel ($50)', category: 'Servicio Proveedor', montoEstimado: 50 }];

    const updated = applyLaundryCosts(original, 80, [{ nombreServicio: 'Mantelería', idServicioCatalogo: 'mantel' } as any]);

    expect(updated[0].montoEstimado).toBe(500);
    expect(original[0].montoEstimado).toBe(50);
  });
