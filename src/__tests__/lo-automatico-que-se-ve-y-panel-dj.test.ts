...
describe('Lo automático que se ve y Panel DJ', () => {
  ...
  describe('Bloque 1: Tareas Automáticas declaradas y estado real', () => {
    it('declara las tareas principales sin jerga', () => {
      expect(TAREAS_AUTOMATICAS.length).toBeGreaterThanOrEqual(7);
      ...
      expect(ids).toContain('recordatorio-a-los-invitados');
    ...
    it('devuelve "nunca" si no hay marcas previas guardadas', async () => {
      ...
      expect(estados).toHaveLength(TAREAS_AUTOMATICAS.length);
