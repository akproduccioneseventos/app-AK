...
function agentRole(agentType: AkAgentType): string {
  const roles: Record<AkAgentType, string> = {
    ...
    fiesta: 'Sos el agente de una fiesta concreta. Revisás pendientes, prioridades, decisiones, módulos incompletos, pagos, contrato, invitados, comida, decoración, discoteca, fotografía y tareas.',
    ...
    central: 'Sos el encargado general de la aplicación. Coordinás a todos los agentes y derivás al especialista correcto.',
  };
...
function fallbackMemory(agentType: AkAgentType, input: AkMultiAgentInput) {
  return {
    ...
    displayName: agentName(agentType),
...
function fallbackResponse(agentType: AkAgentType, context: any, error?: unknown) {
  const items = context?.briefing?.items?.slice(0, 5) || [];
  const diagnostics = items.length
    ? items.map((item: any, index: number) => `${index + 1}. ${item.title}: ${item.detail}`).join('\n')
  ...
  return [
    ...
    agentType === 'fiesta' ? '- Revisar tareas, pagos, contrato y personal de esta fiesta.' :
      agentType === 'fiestas_general' ? '- Revisar primero las fiestas próximas y atrasadas.' :
      agentType === 'contable' ? '- Revisar pagos pendientes, saldos y presupuestos recientes.' :
      agentType === 'marketing' ? '- Crear contenido solo con datos reales cargados.' :
      agentType === 'comercial' ? '- Revisar leads pendientes y presupuestos sin respuesta.' :
      agentType === 'secretaria' ? '- Revisar agenda, llamadas y recordatorios.' :
      '- Elegir el agente correcto y revisar alertas generales.',
    note,
  ].filter(Boolean).join('\n');
}
...
export async function runMultiAgent(input: AkMultiAgentInput): Promise<AkMultiAgentOutput> {
  ...
  const context = await buildContext(input, agentType).catch(() => ({ fiesta: null, briefing: null, text: 'No pude cargar contexto completo.' }));
