...
function FotografiaContent() {
    ...
    const loadData = useCallback(async (showLoading = true) => {
        ...
        try {
            ...
            if (!auth.authorized) {
                setAccessError('Acceso no autorizado o token vencido. Verificá el enlace recibido.');
                setIsLoading(false);
                return;
            }
                    ...
                    {currentItem && (
                        <div className="space-y-4 py-2">
                           ...
                           <div className="space-y-1">
                            ...
                            <div className="space-y-1">
                                <Label htmlFor="item-link">Enlace del Álbum Digital Oficial (Marca Blanca / Dominio AK)</Label>
                                <Input id="item-link" type="url" value={currentItem.linkEntrega || ''} onChange={e => setCurrentItem(p => p ? {...p, linkEntrega: e.target.value} : null)} placeholder="https://galeria.akproducciones.com/..."/>
                           </div>
                        </div>
                    )}
                    <DialogFooter>
