            ...
            {fiesta.presupuestoId && (
              <div className="mt-2 mb-3 p-2 border border-primary/30 rounded-md bg-primary/5 flex items-center justify-between gap-2 flex-wrap">
                ...
                <div className="flex gap-2">
                  <Button asChild variant="secondary" size="sm"><Link href={`/presupuestos/${fiesta.presupuestoId}/ver`}><FileText className="w-4 h-4 mr-1.5"/>Ver Presupuesto</Link></Button>
                  <Button asChild variant="default" size="sm" className="bg-primary text-white"><Link href={`/presupuestos/${fiesta.presupuestoId}/ver?imprimir=1`}><Printer className="w-4 h-4 mr-1.5"/>Generar e imprimir presupuesto</Link></Button>
                </div>
              </div>
            )}
            <div className="flex flex-wrap gap-2 mt-2">
                {!fiesta.presupuestoId && (
                   <Button asChild variant="secondary" size="sm"><Link href={`/presupuestos/nuevo/crear?fiestaId=${fiestaId}`}><ListChecks className="w-4 h-4 mr-1.5"/>Crear Presupuesto</Link></Button>
                )}
                <Button asChild variant="default" size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white"><Link href={`/fiestas/nueva/gestion-documental/recibo-pago?fiestaId=${fiestaId}`}><Receipt className="w-4 h-4 mr-1.5"/>Recibo de Pago PDF</Link></Button>
                <Button asChild variant="secondary" size="sm"><Link href={`/fiestas/nueva/gestion-documental/contrato-servicio?fiestaId=${fiestaId}`}><FileSignature className="w-4 h-4 mr-1.5"/>Borrador Contrato Servicio</Link></Button>
                <Button asChild variant="secondary" size="sm"><Link href={`/fiestas/nueva/gestion-documental/contrato-salon?fiestaId=${fiestaId}`}><Building2 className="w-4 h-4 mr-1.5"/>Borrador Contrato Salón</Link></Button>
                <Button asChild variant="secondary" size="sm"><Link href={`/fiestas/nueva/gestion-documental/cambio-fecha?fiestaId=${fiestaId}`}><FileArchive className="w-4 h-4 mr-1.5"/>Generar Cambio de Fecha</Link></Button>
                <Button asChild variant="destructive" size="sm"><Link href={`/fiestas/nueva/gestion-documental/cancelacion-contrato?fiestaId=${fiestaId}`}><FileX className="w-4 h-4 mr-1.5"/>Generar Cancelación</Link></Button>
                <Button asChild variant="secondary" size="sm"><Link href={`/fiestas/nueva/gestion-documental/cancelacion-parcial?fiestaId=${fiestaId}`}><FileX className="w-4 h-4 mr-1.5"/>Cancelación Parcial</Link></Button>
