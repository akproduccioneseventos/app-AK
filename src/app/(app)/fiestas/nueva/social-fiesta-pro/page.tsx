      ...
      <div className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <section className="grid gap-6 lg:grid-cols-[1.4fr_0.8fr]">
          <Card><CardContent className="space-y-5 p-6 sm:p-8">
            ...
            <p className="max-w-3xl text-base text-muted-foreground">Este panel une la invitación web, la red social privada (tipo Facebook), los juegos interactivos, sorteos y la pantalla gigante LED. Todo controlado para el operador.</p>
            <div className="grid gap-3 sm:grid-cols-3">
              <Button asChild variant="outline" className="h-auto justify-start gap-3 whitespace-normal p-4 text-left"><Link href={linkFor('/fiestas/nueva/muro-social', fiestaId)}><Monitor className="h-5 w-5 shrink-0 text-violet-700" /><span><span className="block font-semibold">Muro y Pantalla LED</span><span className="block text-xs font-normal text-muted-foreground">Controlar la playlist de pantalla y moderar los posts.</span></span></Link></Button>
              <Button asChild variant="outline" className="h-auto justify-start gap-3 whitespace-normal p-4 text-left"><Link href={linkFor('/fiestas/nueva/pagina-web', fiestaId)}><QrCode className="h-5 w-5 shrink-0" /><span><span className="block font-semibold">Invitación Web</span><span className="block text-xs font-normal text-muted-foreground">RSVP y accesos del invitado.</span></span></Link></Button>
              <Button asChild variant="outline" className="h-auto justify-start gap-3 whitespace-normal p-4 text-left"><Link href={linkFor('/fiestas/nueva/en-vivo', fiestaId)}><Sparkles className="h-5 w-5 shrink-0" /><span><span className="block font-semibold">Evento en Vivo</span><span className="block text-xs font-normal text-muted-foreground">Aprobación instantánea de fotos el día del evento.</span></span></Link></Button>
              <Button asChild variant="outline" className="h-auto justify-start gap-3 whitespace-normal p-4 text-left"><a href={`/evento/social/${fiestaId}`} target="_blank" rel="noopener noreferrer"><Users className="h-5 w-5 shrink-0 text-emerald-600" /><span><span className="block font-semibold">Red Social Celular</span><span className="block text-xs font-normal text-muted-foreground">Vista tipo Facebook para el celular del invitado.</span></span></a></Button>
              <Button asChild variant="outline" className="h-auto justify-start gap-3 whitespace-normal p-4 text-left"><a href={`/evento/muro-en-vivo/${fiestaId}`} target="_blank" rel="noopener noreferrer"><Monitor className="h-5 w-5 shrink-0 text-amber-600" /><span><span className="block font-semibold">Pantalla Gigante</span><span className="block text-xs font-normal text-muted-foreground">Salida limpia de video para proyector o pantalla del salón.</span></span></a></Button>
            </div>
          </CardContent></Card>
