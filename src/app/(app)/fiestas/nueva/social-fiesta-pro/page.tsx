...
import { ArrowLeft, Camera, CheckCircle2, Flame, Gamepad2, Heart, MessageCircle, Monitor, QrCode, ShieldCheck, Sparkles, Trophy, Users, ExternalLink } from 'lucide-react';
      ...
      <div className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <section className="grid gap-6 lg:grid-cols-[1.4fr_0.8fr]">
          <Card><CardContent className="space-y-5 p-6 sm:p-8">
            ...
            <h1 className="text-3xl font-bold tracking-tight text-foreground sm:text-4xl">{eventName}</h1>
          ...
          <Card><CardHeader><CardTitle>Estado del motor</CardTitle></CardHeader><CardContent className="space-y-3 text-sm">
            <div className="flex items-center justify-between gap-3 rounded-md bg-muted/30 p-3"><span>Etapa</span><strong>{getFiestaSocialStageLabel(config.stage)}</strong></div>
            <div className="flex items-center justify-between gap-3 rounded-md bg-muted/30 p-3"><span>Privacidad</span><strong>Solo invitados</strong></div>
            <div className="flex items-center justify-between gap-3 rounded-md bg-muted/30 p-3"><span>Moderacion</span><strong>Segura</strong></div>
          </CardContent></Card>
        </section>

        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <Card><CardContent className="flex items-center gap-4 p-5"><MessageCircle className="h-5 w-5 text-violet-700" /><div><p className="text-sm text-muted-foreground">Contenido inicial</p><p className="text-2xl font-semibold text-foreground">{seedPosts.length}</p><p className="text-xs text-muted-foreground">avisos AK listos</p></div></CardContent></Card>
          <Card><CardContent className="flex items-center gap-4 p-5"><Monitor className="h-5 w-5 text-violet-700" /><div><p className="text-sm text-muted-foreground">Pantalla</p><p className="text-2xl font-semibold text-foreground">{slides.length || 1}</p><p className="text-xs text-muted-foreground">slides generados</p></div></CardContent></Card>
          <Card><CardContent className="flex items-center gap-4 p-5"><Gamepad2 className="h-5 w-5 text-violet-700" /><div><p className="text-sm text-muted-foreground">Juegos</p><p className="text-2xl font-semibold text-foreground">{gameSet.length}</p><p className="text-xs text-muted-foreground">desafios por tipo de evento</p></div></CardContent></Card>
          <Card><CardContent className="flex items-center gap-4 p-5"><ShieldCheck className="h-5 w-5 text-violet-700" /><div><p className="text-sm text-muted-foreground">Aprobados</p><p className="text-2xl font-semibold text-foreground">{moderation.bigScreen}</p><p className="text-xs text-muted-foreground">aptos para pantalla</p></div></CardContent></Card>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-3">
          <Card className="lg:col-span-2"><CardHeader><CardTitle>Lo que ve la gente en la fiesta</CardTitle></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2">
            {(slides.length ? slides : [fallbackSlide]).slice(0, 4).map(slide => <div key={slide.id} className="rounded-lg border bg-card p-4"><Badge variant="outline">{slide.mode.replace(/_/g, ' ')}</Badge><h3 className="mt-3 font-semibold text-foreground">{slide.title}</h3>{slide.subtitle && <p className="mt-2 text-sm text-muted-foreground">{slide.subtitle}</p>}<p className="mt-3 text-xs text-muted-foreground">{Math.round(slide.durationMs / 1000)} segundos en pantalla</p></div>)}
          </CardContent></Card>
          <Card><CardHeader><CardTitle>Prompts listos</CardTitle></CardHeader><CardContent className="space-y-3">{prompts.slice(0, 5).map(prompt => <div key={prompt.id} className="rounded-lg border bg-card p-3"><p className="font-medium text-foreground">{prompt.title}</p><p className="mt-1 text-sm text-muted-foreground">{prompt.prompt}</p></div>)}</CardContent></Card>
        </section>

        <section className="grid gap-6 lg:grid-cols-2">
          <Card><CardHeader><CardTitle>Juegos y sorteos activables</CardTitle></CardHeader><CardContent className="space-y-3">{gameSet.map(game => <div key={game.id} className="flex gap-3 rounded-lg border bg-card p-4"><Trophy className="mt-1 h-5 w-5 shrink-0 text-amber-600" /><div><p className="font-semibold text-foreground">{game.title}</p><p className="text-sm text-muted-foreground">{game.description}</p></div></div>)}</CardContent></Card>
          <Card><CardHeader><CardTitle>Reglas simples para que no se rompa</CardTitle></CardHeader><CardContent className="space-y-3">{[['Cliente', 'Puede ver, elegir musica, coordinar reuniones y enviar material.'], ['Invitado', 'Usa el celular: confirma, sube fotos, deja mensajes y participa.'], ['Operador', 'Aprueba u oculta antes de mostrar en pantalla gigante.'], ['AK', 'Mantiene presencia sutil: se ve profesional sin invadir la fiesta.']].map(([title, text]) => <div key={title} className="flex gap-3 rounded-lg bg-muted/30 p-4"><Users className="mt-1 h-5 w-5 shrink-0 text-muted-foreground" /><div><p className="font-semibold text-foreground">{title}</p><p className="text-sm text-muted-foreground">{text}</p></div></div>)}</CardContent></Card>
        </section>
