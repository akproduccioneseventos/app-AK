...
export function getFiestaTimeRange(
  fechaEvento: string | undefined,
  horaInicio: string | undefined,
  horaFin: string | undefined
): { startMs: number; endMs: number } | null {
  if (!fechaEvento || !horaInicio || !horaFin) return null;
  const baseDateStr = fechaEvento.slice(0, 10);
  ...
  let endMins = parseTime(horaFin);
  if (endMins <= startMins || (endMins < 8 * 60)) {
    // Si termina al otro dia, por ej a las 04:00 o 06:00
    endMins += 24 * 60;
  }

  return {
    startMs: baseDate.getTime() + startMins * 60 * 1000,
    endMs: baseDate.getTime() + endMins * 60 * 1000,
  };
}
...
export async function verificarAgendaEmpleado(
  ...
  horaFin: string,
  exceptoFiestaId?: string
): Promise<{ superpuestas: { nombre: string; horaInicio: string; horaFin: string }[], mismoDia: { nombre: string; horaInicio: string; horaFin: string }[] }> {
  const result = { superpuestas: [] as any[], mismoDia: [] as any[] };
  const targetRange = getFiestaTimeRange(fechaEvento, horaInicio, horaFin);
  if (!empleadoId || !fechaEvento) return result;
  const dia = fechaEvento.slice(0, 10);
  ...
  try {
    ...
    for (const fiesta of fiestas) {
      ...
      const fFecha = String(fiesta?.configuracion?.fechaEvento ?? '').slice(0, 10);
      const fHoraInicio = fiesta?.configuracion?.horaInicio ?? '';
      const fHoraFin = fiesta?.configuracion?.horaFin ?? '';
      
      const isMismoDia = fFecha === dia;
      const tieneEmpleado = (fiesta?.personalAsignado ?? []).some((p: any) => p?.empleadoId === empleadoId);
      
      if (tieneEmpleado) {
        const nombre = fiesta?.configuracion?.nombreEvento || 'una fiesta sin nombre';
        const fRange = getFiestaTimeRange(fFecha, fHoraInicio, fHoraFin);
        
        let superpuesta = false;
        if (targetRange && fRange) {
          // Solapamiento: start1 < end2 AND start2 < end1
          if (targetRange.startMs < fRange.endMs && fRange.startMs < targetRange.endMs) {
            superpuesta = true;
          }
        } else if (isMismoDia) {
          // Si faltan horas pero es el mismo dia, asumimos que puede haber overlap por seguridad
          // aunque el requerimiento decia si "es el mismo dia avisar". 
          // Si no hay horas, no podemos probar solapamiento, cae en mismoDia.
        }

        if (superpuesta) {
          result.superpuestas.push({ nombre, horaInicio: fHoraInicio, horaFin: fHoraFin });
        } else if (isMismoDia) {
          result.mismoDia.push({ nombre, horaInicio: fHoraInicio, horaFin: fHoraFin });
        }
      }
    }
  } catch {}
