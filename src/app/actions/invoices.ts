...
function getInvoiceStatusAfterPayment(invoice: Invoice, totalPaid: number): Invoice['status'] {
  const total = roundMoney(invoice.totalAmount);
  if (total > 0 && totalPaid >= total - MONEY_TOLERANCE) return 'Paid';
  if (totalPaid > 0 && invoice.status !== 'Overdue') return invoice.status === 'Draft' ? 'Sent' : invoice.status;
  ...
  try {
    ...
    if ('id' in invoiceDataInput && invoiceDataInput.id) {
      ...
      const updatedItems = (dataToUpdate.items || invoices[index].items).map((item, idx) => ({
        ...
        total: roundMoney(item.quantity) * roundMoney(item.unitPrice),
      ...
      if (totalPaid > totalAmount + MONEY_TOLERANCE) {
        return { success: false, error: 'Los pagos registrados superan el total de la factura. Revisá los pagos antes de bajar el total.' };
      }
      ...
      invoices[index] = {
        ...
        taxAmount,
        totalAmount,
        status: getInvoiceStatusAfterPayment({ ...invoices[index], totalAmount } as Invoice, totalPaid),
      };
...
export async function registerBookingDeposit(data: { fiestaId: string; amount: number; method: string; date: string }): Promise<{ success: boolean; invoiceId?: string; error?: string }> {
  try {
    ...
    if (invIdx !== -1) {
      ...
      freshInvoices[invIdx].status = 'Paid';
...
export async function addPaymentToInvoice(
  ...
  if (amount > balance + MONEY_TOLERANCE) {
    return { success: false, error: `El pago supera el saldo pendiente. Saldo: ${balance.toLocaleString('es-UY')} UYU.` };
  }
  ...
  const newStatus = getInvoiceStatusAfterPayment(invoice, totalPaid);
