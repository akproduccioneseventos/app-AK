                    ...
                    <TableRow key={customer.id}>
                      ...
                      <TableCell className="text-right print:hidden">
                        <div className="flex items-center justify-end gap-2">
                          {customer.phone && (
                            <a
                              href={`https://wa.me/${customer.phone.replace(/\D/g, '')}?text=${encodeURIComponent(`Hola ${customer.name || customer.companyName || ''}! Te contactamos de AK Producciones.`)}`}
                              target="_blank"
                              rel="noopener noreferrer"
                            >
                              <Button variant="outline" size="icon" aria-label={`WhatsApp a ${customer.companyName || customer.name}`} title="Enviar WhatsApp" className="text-[#25D366] border-[#25D366] hover:bg-[#25D366] hover:text-white">
