...
import { ArrowLeft, Package, PackagePlus, Edit, Trash2, Loader2, AlertTriangle, Search, DollarSign, Tag, StickyNote, Printer, Share2 } from 'lucide-react';
...
export default function BebidasPage() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const beverageItems = data.filter(s => s.tipoItem === 'Bebida (Insumo)');
      setAllItems(beverageItems);
      setFilteredItems(beverageItems);
    } catch (err: any) {
      setError("No se pudo cargar el inventario de bebidas.");
  ...
  const handleDelete = async (id: string, nombreItem?: string) => {
    ...
    try {
      ...
      if (result.success) {
        toast({ title: "Ítem Eliminado", description: `La bebida "${nombreItem || id}" ha sido eliminada.` });
        fetchItems(); 
      } else {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
         ...
         <div className="flex gap-2 flex-wrap">
            <Link href="/empresa/bebidas/nuevo" passHref>
      ...
      <CardDescription>Gestiona el inventario de bebidas que utilizas en tus eventos.</CardDescription>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-10"><Loader2 className="w-12 h-12 animate-spin text-primary" /><p className="ml-3 text-muted-foreground">Cargando bebidas...</p></div>
      ) : error ? (
        ...
        <Card className="shadow-md"><CardContent className="p-6 text-center text-muted-foreground"><Search className="w-16 h-16 mx-auto mb-4 opacity-30" />{searchTerm ? "No se encontraron bebidas que coincidan." : "El inventario de bebidas está vacío."}</CardContent></Card>
      ) : (
