...
export default function IngredientesPage() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      setError("No se pudo cargar el inventario de ingredientes.");
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
         ...
         <div className="flex gap-2 flex-wrap">
            <Link href="/empresa/ingredientes/nuevo" passHref>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-10"><Loader2 className="w-12 h-12 animate-spin text-primary" /><p className="ml-3 text-muted-foreground">Cargando ingredientes...</p></div>
      ) : error ? (
        ...
        <Card className="shadow-md"><CardContent className="p-6 text-center text-muted-foreground"><Search className="w-16 h-16 mx-auto mb-4 opacity-30" />{searchTerm ? "No se encontraron ingredientes que coincidan." : "El inventario de ingredientes está vacío."}</CardContent></Card>
      ) : (
