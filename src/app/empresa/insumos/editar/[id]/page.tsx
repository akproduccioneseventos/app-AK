...
import { getInsumoById, saveInsumo, deleteInsumo } from '@/app/actions/insumos';
...
export default function EditarInsumoPage({ params: paramsProp }: { params: { id: string } }) {
  ...
  const loadItem = useCallback(async () => {
    ...
    setNotFound(false);
    try {
      const loadedItem = await getInsumoById(itemIdFromParams);
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const result = await deleteInsumo(item.id);
      if (result.success) {
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <Select value={formData.categoria || ''} onValueChange={(value) => handleCategoryChange(value as AnyCategoria)} required disabled={isSaving}>
                  ...
                  <SelectContent className="max-h-60">{ALL_CATEGORIAS_INSUMO.map(cat => (<SelectItem key={cat} value={cat}>{cat}</SelectItem>))}</SelectContent>
                </Select>
              </div>
            </div>
