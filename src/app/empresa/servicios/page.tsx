...
export default function ServiciosPage() {
  ...
  const itemsAgrupadosPorCategoria = useMemo(() => {
    return filteredItems.reduce((acc, item) => {
      const categoria = item.categoria || 'Otros servicios';
  ...
  return (
    <div className="space-y-6">
      ...
      <CardDescription>Gestiona los servicios que ofreces a tus clientes en los presupuestos, con sus precios de venta definidos.</CardDescription>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-10"><Loader2 className="w-12 h-12 animate-spin text-primary" /><p className="ml-3 text-muted-foreground">Cargando servicios...</p></div>
      ) : error ? (
        ...
        <Card className="shadow-md"><CardContent className="p-6 text-center text-muted-foreground"><Search className="w-16 h-16 mx-auto mb-4 opacity-30" />{searchTerm ? "No se encontraron servicios que coincidan." : "El catálogo de servicios está vacío."}</CardContent></Card>
      ) : (
                    ...
                    return (
                      <Card key={item.id} className="bg-muted/30 hover:shadow-sm transition-shadow">
                        ...
                        <CardContent className="px-3 pb-3 text-sm space-y-1">
                            <p><span className="text-muted-foreground">Precio Venta: </span><span className="font-bold text-primary/90">{getPriceDisplay(item)}</span></p>
