...
import { Download, Share2, Heart, ArrowLeft, Loader2, Play, ChevronLeft, ChevronRight, X, ImageIcon, Camera, Sparkles } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        {isLoading && posts.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center px-4">
            <div className="h-16 w-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center mb-5 text-amber-400">
              <Camera className="w-8 h-8" />
            </div>
            <h3 className="text-xl font-bold text-white mb-2">Aún no hay fotos publicadas</h3>
            <p className="text-sm text-zinc-400 max-w-sm mb-6">
              ¡Sé el primero en compartir un momento! Podés subir fotos desde tu celular o acercarte a la fotocabina de la fiesta.
            </p>
            <div className="flex flex-wrap items-center justify-center gap-3">
              <Link
                href={`/evento/social/${fiestaId}`}
                className="inline-flex items-center gap-2 px-6 py-3 bg-amber-500 hover:bg-amber-400 text-zinc-950 font-bold text-sm rounded-full shadow-lg shadow-amber-500/20 transition-all"
              >
                <Camera className="w-4 h-4" />
                Subí tu primera foto
              </Link>
              <Link
                href={`/evento/fotocabina/${fiestaId}`}
                className="inline-flex items-center gap-2 px-5 py-3 bg-white/10 hover:bg-white/15 text-white font-semibold text-sm rounded-full border border-white/10 transition-all"
              >
                <Sparkles className="w-4 h-4 text-amber-300" />
                Ir a la estación de fotos
              </Link>
            </div>
          </div>
        ) : (
