...
export default function EditarMenuEspecificoPage({ params }: { params: { menuId: string } }) {
      ...
      try {
        ...
        if (loadedMenu) {
          ...
          setMenuItems(loadedMenu.items.map(item => ({
            ...item,
            // Recalculate totalDishCost and costPerPortion based on stored ingredients and basePortions
            // This ensures consistency if stored values were derived differently or are missing
            ...
            costPerPortion: (item.basePortions && item.basePortions > 0 && item.ingredients.length > 0)
                              ...
                              : undefined,
            allergens: item.allergens || '', // Ensure allergens is a string
          })));
        } else {
          setNotFound(true);
  ...
  const handleSaveChanges = async () => {
    ...
    const updatedMenuData: FullMenu = {
      ...
      items: menuItems.map(item => ({ // Ensure all fields are correctly typed for saving
        ...item,
        totalDishCost: item.ingredients.reduce((sum, ing) => sum + (ing.cost || 0), 0), // Recalculate
        costPerPortion: (item.basePortions && item.basePortions > 0 && item.ingredients.length > 0)
                          ? item.ingredients.reduce((sum, ing) => sum + (ing.cost || 0), 0) / item.basePortions
                          : undefined,
    ...
    try {
      ...
      if (result.success && result.menu) {
        ...
        setMenuItems(result.menu.items); // Refresh items with potentially re-calculated costs/portions from server
      } else {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <Card className="shadow-lg">
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="menu-name" value={menuName} onChange={(e) => setMenuName(e.target.value)} className="text-base p-3" disabled={isSaving || isDeleting} required/>
          </div>
           <div className="space-y-2">
            <Label htmlFor="menu-description" className="text-base">Descripción Corta (Opcional)</Label>
            <Input id="menu-description" value={menuDescription} onChange={(e) => setMenuDescription(e.target.value)} className="text-base p-3" disabled={isSaving || isDeleting}/>
          </div>
          <div className="space-y-2">
            <Label htmlFor="menu-template-type-edit" className="text-base">Tipo de Plantilla de Menú</Label>
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Define el nombre, categoría, porciones, alérgenos e ingredientes. Para modificar, elimina y vuelve a añadir.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2"><Label htmlFor="new-item-type-edit">Categoría del Plato *</Label><Select value={newItemType} onValueChange={(value) => setNewItemType(value as MenuItem['type'])} required disabled={isSaving || isDeleting}><SelectTrigger id="new-item-type-edit"><SelectValue placeholder="Seleccionar" /></SelectTrigger><SelectContent><SelectItem value="Entrada">Entrada</SelectItem><SelectItem value="Plato Principal">Plato Principal</SelectItem><SelectItem value="Postre">Postre</SelectItem><SelectItem value="Bebida">Bebida</SelectItem></SelectContent></Select></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2"><Label htmlFor="new-item-portions-edit">Rinde para (porciones base)</Label><Input id="new-item-portions-edit" type="number" value={newItemBasePortions ?? ''} onChange={(e) => setNewItemBasePortions(e.target.value ? parseInt(e.target.value) : undefined)} min="1" disabled={isSaving || isDeleting}/></div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 mb-3 items-end">
              ...
              <div className="space-y-1"><Label htmlFor="ing-cost-edit" className="text-xs">Costo Ing. *</Label><Input id="ing-cost-edit" type="number" value={ingredientCost} onChange={e => setIngredientCost(e.target.value)} className="text-sm p-2 h-9" step="any" disabled={isSaving || isDeleting}/></div>
            </div>
            ...
            {currentDishIngredients.length > 0 && (<div className="mt-4 space-y-2"><h4 className="text-sm font-medium">Ingredientes añadidos:</h4><ScrollArea className="h-[100px] border rounded-md p-2 bg-muted/30"><ul className="text-sm">{currentDishIngredients.map(ing => (<li key={ing.id} className="flex justify-between items-center py-1 border-b last:border-b-0"><span>{ing.name} ({ing.quantity} {ing.unit}) - Costo: ${ing.cost.toFixed(2)}</span><Button variant="ghost" size="icon" onClick={() => handleRemoveIngredientFromCurrentDish(ing.id)} className="h-6 w-6 text-destructive hover:bg-destructive/10" disabled={isSaving || isDeleting}><Trash2 className="w-3 h-3" /></Button></li>))}</ul></ScrollArea><p className="text-sm text-right font-medium">Costo Total Ingredientes: ${currentDishTotalCost.toFixed(2)}</p></div>)}
          </div>
          <Separator />
