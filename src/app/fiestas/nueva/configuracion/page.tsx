      ...
      <form onSubmit={handleSubmit}>
        <Card className="shadow-lg">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2"><Label htmlFor="info-parking" className="text-base">🅿️ Info de Estacionamiento</Label><Textarea id="info-parking" value={config.infoParking || ''} onChange={(e) => handleChange('infoParking', e.target.value)} placeholder="Ej: Estacionamiento gratuito en la puerta. Calle lateral disponible." rows={2} className="text-base p-3" disabled={isSaving}/></div>
              <div className="space-y-2"><Label htmlFor="accesibilidad" className="text-base">♿ Accesibilidad</Label><Textarea id="accesibilidad" value={config.accesibilidad || ''} onChange={(e) => handleChange('accesibilidad', e.target.value)} placeholder="Ej: Rampa de acceso en entrada lateral. Baño adaptado disponible." rows={2} className="text-base p-3" disabled={isSaving}/></div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2"><Label htmlFor="protocolo-lluvia" className="text-base">🌧️ Protocolo de Lluvia</Label><Textarea id="protocolo-lluvia" value={config.protocoloLluvia || ''} onChange={(e) => handleChange('protocoloLluvia', e.target.value)} placeholder="Ej: En caso de lluvia, la recepción se traslada al salón interno." rows={2} className="text-base p-3" disabled={isSaving}/></div>
              <div className="space-y-2"><Label htmlFor="contacto-urgencias" className="text-base">📞 Contacto Urgencias (Día del Evento)</Label><Input id="contacto-urgencias" value={config.contactoUrgencias || ''} onChange={(e) => handleChange('contactoUrgencias', e.target.value)} placeholder="Ej: +598 99 123 456 (Coordinador AK)" className="text-base p-3" disabled={isSaving}/></div>
            </div>
            <div className="space-y-2"><Label htmlFor="mapa-url" className="text-base">🗺️ URL del Mapa del Salón</Label><Input id="mapa-url" type="url" value={config.mapaUrl || ''} onChange={(e) => handleChange('mapaUrl', e.target.value)} placeholder="https://... (enlace al mapa o plano del salón)" className="text-base p-3" disabled={isSaving}/></div>
          </CardContent>
