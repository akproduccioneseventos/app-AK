...
const formatDate = (dateString?: string) => {
  ...
  try {
    ...
    // Handle timezone offset if date is ISO string
...
function ContratoServicioContent() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!fiestaData.configuracion.clienteId) {
        ...
        setFiesta(fiestaData); // set fiesta even if there's an error to allow back navigation
        setIsLoading(false);
        return;
      }
      ...
      if (!clienteData) {
        setError(`No se encontraron los datos para el cliente asignado.`);
      }
  ...
  if (!fiesta) {
    return <div className="text-center py-10"><p className="text-muted-foreground">No se encontró información del evento.</p></div>;
  }
