...
import { ArrowLeft, Printer as PrinterIcon, Share2, AlertTriangle, FileArchive, FileX } from 'lucide-react';
...
function ContratoServicioContent() {
  ...
  const handleShare = async () => {
    const shareData = {
      title: `Contrato de Servicio - ${fiesta?.configuracion.nombreEvento}`,
      text: `Borrador del contrato de servicio para el evento.`,
    ...
    <div className="bg-gray-100 print:bg-white py-6 print:py-0 font-sans">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <Card className="print:hidden">
            <CardHeader>
                <CardTitle>Anexos del Contrato</CardTitle>
                <CardDescription>Genera documentos adicionales relacionados a este contrato.</CardDescription>
            </CardHeader>
            <CardContent className="flex flex-wrap gap-4">
                <Link href={`/fiestas/nueva/gestion-documental/cambio-fecha?fiestaId=${fiestaId}`} passHref>
                    <Button variant="secondary"><FileArchive className="w-4 h-4 mr-2"/>Constancia de Cambio de Fecha</Button>
                </Link>
                <Link href={`/fiestas/nueva/gestion-documental/cancelacion-contrato?fiestaId=${fiestaId}`} passHref>
                    <Button variant="destructive"><FileX className="w-4 h-4 mr-2"/>Constancia de Cancelación</Button>
                </Link>
            </CardContent>
        </Card>
        
        <div className="bg-white shadow-xl print:shadow-none p-6 md:p-10 print:p-2">
            <header className="mb-6 print:mb-4">
                ...
                <h1 className="text-xl font-bold text-center">CONTRATO DE PRESTACIÓN DE SERVICIOS PARA EVENTOS</h1>
            </header>
            
            <div className="prose prose-sm print:prose-xs max-w-none text-justify">
            <p>En la ciudad de Salto, a los {today}, entre: Por una parte, <strong>{companyInfo?.companyName || 'AK PRODUCCIONES EVENTOS'}</strong>, RUT: {companyInfo?.companyTaxId || '____________'}, representado en este acto por el Tec. Alexander Knuth, C.I. 46173508, con domicilio en {companyInfo?.companyAddress || '____________'}, correo electrónico {companyInfo?.companyContact || '____________'}, en adelante el “PRESTADOR DEL SERVICIO”. Por otra parte, <strong>{cliente?.name || '________________________'}</strong>, con domicilio en ________________________, cédula de identidad N° <strong>{cliente?.taxId || '_______________________'}</strong>, número de contacto <strong>{cliente?.phone || '_______________________'}</strong>, en adelante el “CLIENTE”. Ambas partes acuerdan celebrar el presente contrato, sujeto a los términos y condiciones que se establecen a continuación:</p>

            <p><strong>PRIMERA: OBJETO</strong> El presente contrato tiene por objeto la prestación de servicios para la organización y realización de una fiesta o evento en la fecha <strong>{formatDate(fiesta.configuracion.fechaEvento)}</strong>, con una duración aproximada de siete (7) horas desde el inicio del evento como máximo, cobrándose un monto de cinco mil pesos por cada hora extra, conforme a las condiciones establecidas en el presupuesto adjunto, el cual forma parte integral del presente contrato. La prestación de los servicios se llevará a cabo en el salón <strong>{fiesta.configuracion.nombreLugar || '____________'}</strong>, cuyo costo será asumido exclusivamente por el CLIENTE.</p>
            
            <p><strong>SEGUNDA: FORMA DE PAGO:</strong> El CLIENTE abonará el precio total estipulado en el presupuesto adjunto conforme a las siguientes modalidades: a) Seña inicial: al momento de la firma del contrato, el CLIENTE abonará la suma de $ 20.000 (pesos uruguayos veinte mil), la cual se imputará al precio total del servicio contratado. En caso de cancelación, dicha suma se considerará parte integrante de la multa prevista en la cláusula de cancelación y no será reintegrada. b) Pagos parciales: luego de la seña inicial, el CLIENTE podrá realizar pagos parciales en los montos y fechas que disponga, los cuales serán imputados al precio total del servicio. c) Saldo final: el saldo pendiente deberá estar totalmente cancelado con una antelación mínima de treinta (30) días corridos a la fecha del evento. El pago podrá realizarse en efectivo, transferencia bancaria a la cuenta designada por el PRESTADOR, o cualquier otro medio válido, entregándose siempre el comprobante correspondiente. El incumplimiento en la cancelación total en el plazo establecido facultará al PRESTADOR a rescindir el contrato, pudiendo exigir al CLIENTE el pago de una multa equivalente al treinta por ciento (30%) del presupuesto total.</p>
            ...
            <p><strong>SÉPTIMA: PAGO</strong> Al momento de la firma del contrato, el CLIENTE abonará la suma de pesos uruguayos veinte mil ($20.000 ) como seña, recibiendo el comprobante correspondiente. El saldo del precio se abonará conforme al plan de pagos acordado, y una vez cancelado el total, AK PRODUCCIONES EVENTOS emitirá el comprobante correspondiente.</p>
            ...
            <div className="mt-16 flex justify-between text-center">
                ...
                <p className="font-semibold">Tec. Alexander Knuth</p>
