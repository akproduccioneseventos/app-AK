...
function VerPresupuestoContent({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="bg-gray-100 print:bg-white py-6 print:py-0 font-sans">
        ...
        <div className="bg-white shadow-xl print:shadow-none p-10 md:p-16 print:p-2 max-w-3xl mx-auto">
                ...
                <div className="border rounded-[1.5rem] overflow-hidden shadow-sm print:border-slate-300">
                    <Table>
                        <TableHeader className="bg-slate-50">
                            <TableRow>
                                <TableHead className="w-3/5 font-black text-[10px] uppercase pl-6">Artículo</TableHead>
                        ...
                        <TableBody>
                            {Object.entries(calculatedValues.itemsAgrupados).map(([categoria, items]) => (
                                <React.Fragment key={categoria}>
                                    <TableRow className="bg-muted/30 border-y"><TableCell colSpan={3} className="font-black text-[10px] uppercase text-primary pl-6 tracking-widest">{categoria}</TableCell></TableRow>
                                    {items.map(item => (
