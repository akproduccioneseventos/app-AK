...
function VerPresupuestoContent({ params }: { params: { id: string } }) {
  ...
  const { itemsAgrupados, costoTotalRegalos, subtotalBruto, descuentoPromocional, totalFinal, showAnnualAdjustmentLegend } = useMemo(() => {
    ...
    const descuentoPromocionalFicticio = valorTotalServiciosMostrado - totalFinalAjustado;
...
```Entendido. Ya veo el problema. El error de referencia que corregí antes era solo un síntoma de un problema más profundo en cómo se calculaban y mostraban los totales. Gracias por el análisis detallado, es extremadamente útil.

He reescrito la lógica de cálculo en la página de visualización de presupuestos (`src/app/presupuestos/[id]/ver/page.tsx`) para asegurar que sea **matemáticamente precisa y transparente**, eliminando la confusa estrategia de "precio ancla" en esta vista para evitar discrepancias.

Así es como funcionará ahora:

1.  **Subtotal Real:** Se calcula sumando el costo de todos los servicios que no son regalos. Este número ahora reflejará exactamente lo que se ve en la tabla.
2.  **Ahorro Total:** Se muestra claramente la suma del **descuento promocional** que hayas configurado más el **valor real de los servicios marcados como regalo**.
3.  **Total a Pagar:** Es el resultado simple y correcto de `Subtotal - Descuento promocional`.

Con estos cambios, ya no habrá diferencias en los cálculos y el presupuesto que le presentas al cliente será coherente y fácil de entender.

<changes>
  <description>Se corrigió un error de cálculo en la página de visualización de presupuestos (`[id]/ver/page.tsx`) que causaba discrepancias entre el subtotal, los descuentos y el total final. La lógica fue reescrita para asegurar que los cálculos sean directos, transparentes y matemáticamente precisos, eliminando la estrategia de "precio ancla" que generaba confusión.</description>
  <change>
    <file>src/app/presupuestos/[id]/ver/page.tsx</file>
    <content><![CDATA[
