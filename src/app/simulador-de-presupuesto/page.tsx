                                ...
                                {Object.entries(serviciosAgrupados).sort(([catA], [catB]) => catA === 'Regalos Incluidos' ? 1 : catB === 'Regalos Incluidos' ? -1 : catA.localeCompare(catB)).map(([categoria, items]) => (
                                    <React.Fragment key={categoria}>
                                        <TableRow className="bg-muted/30 print:bg-gray-50">
                                            <TableCell colSpan={2} className={`${categoria === 'Regalos Incluidos' ? 'text-destructive' : 'text-primary'}`}>
                                        ...
                                        {items.map((item) => (
                                            <TableRow key={item.id}>
                                                ...
                                                <TableCell className={`${item.esRegalo ? 'text-muted-foreground line-through' : ''}`}>{formatCurrency(item.costo)}</TableCell>
                                            </TableRow>
                                        ))}
                                    </React.Fragment>
                                ))}
