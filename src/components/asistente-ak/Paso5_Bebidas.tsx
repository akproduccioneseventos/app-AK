...
export const AsistentePaso5_Bebidas: React.FC<Props> = ({ data, setData, handleNext }) => {
  ...
  useEffect(() => {
    async function loadConfig() {
      ...
      try {
        ...
        const { bebidas } = config.pasos;
        setOpciones(bebidas.opciones);
        setPregunta(bebidas.pregunta);
        setDescripcion(bebidas.descripcion);
      } catch (error) {
  ...
  const handleSelect = (opcion: AsistentePasoOpcion) => {
    ...
    setData(prev => ({ ...prev, bebidas: { ...restOfOpcion, costoPorPersona: opcion.costoPorPersona || 0 }}));
