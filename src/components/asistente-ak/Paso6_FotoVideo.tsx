...
export const AsistentePaso6_FotoVideo: React.FC<Props> = ({ data, setData, handleNext }) => {
  ...
  useEffect(() => {
    async function loadConfig() {
      ...
      try {
        ...
        const { fotoVideo } = config.pasos;
        setOpciones(fotoVideo.opciones);
        setPregunta(fotoVideo.pregunta);
        setDescripcion(fotoVideo.descripcion);
      } catch (error) {
  ...
  const handleSelect = (opcion: AsistentePasoOpcion) => {
    setData(prev => ({ ...prev, fotoVideo: { ...opcion, costoBase: opcion.costoBase || 0 } }));
