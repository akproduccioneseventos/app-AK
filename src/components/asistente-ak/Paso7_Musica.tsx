...
export const AsistentePaso7_Musica: React.FC<Props> = ({ data, setData, handleNext }) => {
  ...
  useEffect(() => {
    async function loadConfig() {
      ...
      try {
        ...
        const { musica } = config.pasos;
        setOpciones(musica.opciones);
        setPregunta(musica.pregunta);
        setDescripcion(musica.descripcion);
      } catch (error) {
  ...
  const handleSelect = (opcion: AsistentePasoOpcion) => {
    setData(prev => ({ ...prev, musica: { ...opcion, costoBase: opcion.costoBase || 0 }}));
