...
export const AsistentePaso8_Reposteria: React.FC<Props> = ({ data, setData, handleNext }) => {
  ...
  useEffect(() => {
    async function loadConfig() {
      ...
      try {
        ...
        const { reposteria } = config.pasos;
        setOpciones(reposteria.opciones);
        setPregunta(reposteria.pregunta);
        setDescripcion(reposteria.descripcion);
      } catch (error) {
  ...
  const handleSelect = (opcion: AsistentePasoOpcion) => {
    setData(prev => ({ ...prev, reposteria: { ...opcion, costoBase: opcion.costoBase || 0, costoPorPersona: opcion.costoPorPersona || 0 } }));
