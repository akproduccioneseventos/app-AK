...
export const AsistentePaso9_Entretenimiento: React.FC<Props> = ({ data, setData, handleNext }) => {
  ...
  useEffect(() => {
    async function loadConfig() {
      ...
      try {
        ...
        const { entretenimiento } = config.pasos;
        setOpciones(entretenimiento.opciones);
        setPregunta(entretenimiento.pregunta);
        setDescripcion(entretenimiento.descripcion);
      } catch (error) {
  ...
  const handleSelect = (opcion: AsistentePasoOpcion) => {
    setData(prev => ({ ...prev, entretenimiento: { ...opcion, costoBase: opcion.costoBase || 0 } }));
