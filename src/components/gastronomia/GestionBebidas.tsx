          ...
          <div className="space-y-3 p-1">
            {editingCategory?.items.map(item => (
              <div key={item.id} className="p-3 border rounded-md grid grid-cols-1 md:grid-cols-3 gap-2 items-end">
                ...
                <div className="space-y-1"><Label htmlFor={`beb-qty-${item.id}`}>Cantidad</Label><Input id={`beb-qty-${item.id}`} type="number" min="0" value={item.cantidadNecesaria || ''} onChange={(e) => handleItemChange(item.id, 'cantidadNecesaria', Math.max(0, Number(e.target.value)))} /></div>
                ...
                <div className="space-y-1 relative"><Label htmlFor={`beb-cost-${item.id}`}>Costo Unitario</Label><Input id={`beb-cost-${item.id}`} type="number" min="0" value={item.costoUnitario || ''} onChange={(e) => handleItemChange(item.id, 'costoUnitario', Math.max(0, Number(e.target.value)))} className="pl-6" disabled={!!item.origenId}/><span className="absolute left-2 top-1/2 mt-1 text-muted-foreground">$</span></div>
