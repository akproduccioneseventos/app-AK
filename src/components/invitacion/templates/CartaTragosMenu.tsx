...
export const MenuComponent: React.FC<MenuComponentProps> = ({ fiesta, carta, logoUrl, inverted, onUpdate, isPreview }) => {
  
  const protagonistaNombre = carta.protagonistaNombre || 'Luciana';
  const numeroPrincipal = carta.numeroPrincipal || 'Mis XV';
  ...
  return (
    <div className={cn("w-full h-full p-0 relative overflow-hidden flex flex-col items-center", inverted && 'print:transform print:rotate-180')} style={{ backgroundColor: '#E9D5FF'}}>
        <Wave className="top-0 z-0" color={carta.paletaColores?.primary || '#a855f7'} />

        <header className="w-full pt-6 px-4 flex justify-between items-center relative z-10">
          ...
          <div className="flex-grow text-center pl-2">
            ...
            <div className="text-4xl mt-1" style={{...textStyle, color: carta.paletaColores?.secondary, fontFamily: "'Dancing Script', cursive"}}>{protagonistaNombre}</div>
            <div className="text-3xl mt-1" style={{...textStyle, color: carta.paletaColores?.secondary, fontFamily: "'Dancing Script', cursive"}}>{numeroPrincipal}</div>
          </div>
        </header>
        ...
        <BottomWave className="bottom-0 z-0" color={carta.paletaColores?.primary || '#a855f7'} />
        
        <footer className="relative z-10 w-full flex flex-col items-center justify-center pb-2 pt-12 text-white">
            {logoUrl && (
