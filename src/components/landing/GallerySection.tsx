...
function getCategoriasAsociadas(
  ...
  const text =
    `${titulo || ""} ${descripcion || ""} ${categoriaOriginal || ""}`.toLowerCase();
  const cats: string[] = [];
  if (
    text.includes("salon") ||
    text.includes("salón") ||
  ...
  if (
    ...
    text.includes("espejo") ||
    text.includes("plataforma") ||
    text.includes("touchpix") ||
    text.includes("fotocabina") ||
    text.includes("fotobina") ||
    text.includes("totem") ||
    text.includes("tótem")
  ) {
  ...
  if (
    ...
    text.includes("film") ||
    text.includes("bogue") ||
    text.includes("exteriores") ||
    text.includes("civil") ||
    text.includes("iglesia") ||
...
export function GallerySection({ images, galeriaFotos }: GallerySectionProps) {
  ...
  const allImages = useMemo<LandingGalleryItem[]>(() => {
    const source =
      (galeriaFotos?.length ?? 0) > 0
        ? galeriaFotos!.map((foto) => {
            const cats = getCategoriasAsociadas(
              foto.titulo || "",
              foto.descripcion || "",
              foto.categoria || foto.servicio || "",
            );
            const subCat = cats.includes("Fotografía")
              ? getSubCategoriaFotografia(
                  foto.titulo || "",
                  foto.descripcion || "",
                  foto.categoria || foto.servicio || "",
                )
    ...
    return uniqueSource.sort(
      (a, b) => (b.destacada ? 1 : 0) - (a.destacada ? 1 : 0),
    );
  }, [galeriaFotos, images]);
  ...
  const prevImage = () =>
    setLightboxIndex((i) =>
      i === null
        ? null
        : (i - 1 + displayedImages.length) % displayedImages.length,
    );
  const nextImage = () =>
    setLightboxIndex((i) =>
      i === null ? null : (i + 1) % displayedImages.length,
    );
  const handleShareWhatsApp = (image: LandingGalleryItem) => {
    const mainCat = image.categorias[0] || "Evento";
    const text = encodeURIComponent(
      `¡Mirá esta foto de ${mainCat} de AK Producciones! ${image.src}`,
    );
  ...
  return (
    <section
      ...
      className="py-24 bg-zinc-950 text-white border-y border-white/5"
    >
      {" "}
          ...
          >
            ...
            >
              ...
              {(displayedImages[lightboxIndex].titulo ||
                (displayedImages[lightboxIndex].categorias &&
                  displayedImages[lightboxIndex].categorias.length > 0) ||
                ...
                <div className="bg-zinc-950/90 border border-white/10 text-white p-4 rounded-2xl mt-3 flex items-center justify-between gap-3">
                    ...
                    {displayedImages[lightboxIndex].categorias &&
                      displayedImages[lightboxIndex].categorias.length > 0 && (
                        <p className="text-xs text-indigo-400 font-bold uppercase tracking-widest mt-1">
                          {displayedImages[lightboxIndex].categorias[0]}
                        </p>
                      )}{" "}
                    {displayedImages[lightboxIndex].descripcion && (
                  ...
                  <button
                    onClick={() =>
                      handleShareWhatsApp(displayedImages[lightboxIndex])
                    }
