...
export function MultiAgentWidget() {
  ...
  const [activeAgent, setActiveAgent] = useState<AkAgentType>('central');
  ...
  const quickPrompts = activeAgent === 'fiesta'
    ? ['¿Qué le falta a esta fiesta?', 'Qué está a revisar', 'Qué hago hoy']
        ...
        {AGENT_ORDER.map(agent => {
          ...
          return (
            ...
            >
              ...
              <span className="flex h-11 w-11 items-center justify-center rounded-full bg-red-600 text-xl text-white shadow-md">
                {item.avatar}
              </span>
            </button>
          );
              ...
              <div className="flex-1 space-y-3 overflow-y-auto p-4">
                {messages.length === 0 && (
                  <div className="rounded-2xl border border-red-100 bg-red-50/60 p-4 text-sm text-slate-700">
                    ...
                    <p className="mt-1">Elegí un agente de la derecha. Cada uno trabaja con su área.</p>
                  </div>
                )}
