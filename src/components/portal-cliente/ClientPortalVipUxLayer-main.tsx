...
function addInternalOverview() {
  ...
  section.innerHTML = `
    <div class="space-y-1">
      <p class="text-lg font-black text-foreground">Portal Cliente VIP ÔÇö estructura correcta</p>
      <p>Este panel configura el portal privado que ve el cliente. La vista p├║blica debe respetar estos mismos nombres y secciones.</p>
    </div>
    <div class="grid gap-2 sm:grid-cols-2">
      ${cards}
    </div>
    <div class="rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
      Pagos, se├▒a y saldo van siempre en <strong>Pagos y documentos</strong>. No deben quedar dentro de tareas o ÔÇ£lo que debe llevarÔÇØ.
    </div>
  `;
...
function addInternalDefaultLinkWarning() {
  ...
  warning.innerHTML = `
    <p class="font-black">Atenci├│n: este portal sigue usando el link de prueba CLIENTE1.</p>
    <p>Antes de enviarlo al cliente, cambi├í el link personalizado por un nombre real del evento. Ejemplo: <strong>cliente-vip-rocio-15</strong>.</p>
  `;
...
function addPublicUnconfiguredWarning() {
  ...
  warning.innerHTML = `
    <div class="rounded-3xl border border-amber-200 bg-amber-50 p-4 shadow-xl text-amber-900 space-y-2">
      <p class="text-base font-black">Portal sin datos reales todav├¡a</p>
      <p class="text-sm leading-relaxed">Este link est├í funcionando, pero el evento todav├¡a no tiene configurados los datos reales del cliente, fecha, lugar, presupuesto o contrato. No conviene enviarlo al cliente hasta completar esos datos desde Planificador.</p>
...
function correctFalsePublicProcessStates() {
  ...
  for (const node of nodes) {
    ...
    const replacements: Array<[string, string]> = [
      ['Pago completado Ô£ô', 'Pago sin configurar'],
      ['Pago completado', 'Pago sin configurar'],
      ['Contrato pendiente de firma', 'Contrato sin configurar'],
      ['Sin confirmaciones a├║n', 'Invitados sin configurar'],
      ['PAGADO', 'PAGO'],
    ];
  ...
  guide.innerHTML = `
    <div class="rounded-3xl bg-white border border-slate-100 shadow-lg p-4 space-y-3">
      ...
      <div class="grid grid-cols-2 gap-2 text-xs text-slate-600">
        <div class="rounded-2xl bg-slate-50 p-3"><strong>Pr├│ximos pasos</strong><br/>Tareas pendientes y mensajes.</div>
        <div class="rounded-2xl bg-slate-50 p-3"><strong>Pagos</strong><br/>Saldo, comprobantes y documentos.</div>
        <div class="rounded-2xl bg-slate-50 p-3"><strong>Evento</strong><br/>Organizaci├│n, men├║, fotos e invitados.</div>
