...
export default function ClientPortalVipUxLayer({ mode }: { mode: 'admin' | 'public' }) {
  useEffect(() => {
    ...
    const observer = new MutationObserver((mutations) => {
      // Ignore mutations generated by internal VIP UX elements
      const hasExternalMutation = mutations.some(m => {
        const target = m.target as HTMLElement;
        if (target?.id?.startsWith('portal-cliente-')) return false;
        if (target?.closest?.('[id^="portal-cliente-"]')) return false;
        return true;
      });
      if (hasExternalMutation && !isApplying) {
        apply();
      }
    });
