...
export function RsvpReminderPanel({ fiestaId }: RsvpReminderPanelProps) {
  ...
  const [data, setData] = useState<{ pendingGuests: Invitado[], messages: RsvpReminderMessage[] } | null>(null);
  ...
  // Placeholder stats - usually these would come from the server
  const totalInvitados = 100;
  const confirmados = 60;
  const pendientes = data.pendingGuests.length;
  const rechazados = 5;
