...
function getSigningKey(): string {
  const configuredKey = process.env.AUTH_SESSION_SECRET || process.env.SESSION_SECRET || process.env.GOOGLE_API_KEY;

  if (configuredKey?.trim()) return configuredKey;
  if (process.env.NODE_ENV !== 'production') return 'ak-dev-session-key';
  throw new Error('A server signing key is required for production sessions.');
}
...
function decodePayload(payloadPart: string): SessionTokenPayload | null {
  try {
    ...
    if (payload.v !== TOKEN_VERSION || typeof payload.iat !== 'number' || typeof payload.exp !== 'number' || typeof payload.nonce !== 'string') return null;
...
export async function createSessionToken(now = Date.now()): Promise<string> {
  ...
  const signaturePart = await sign(payloadPart, getSigningKey());
...
export async function verifySessionToken(token: string | null | undefined, now = Date.now()): Promise<boolean> {
  try {
    ...
    const expectedSignature = await sign(payloadPart, getSigningKey());
