...
const ALLOWED_HISTORY_PLATFORMS: SocialPlatform[] = [
  'Facebook',
  'Instagram',
  'Threads',
  'TikTok',
...
function extractDate(record: UnknownRecord): string | null {
  const keys = [
    'timestamp', 'timestamp_ms', 'created_at', 'createdAt', 'create_time', 'createTime',
    'published_at', 'publishedAt', 'publication_date', 'date', 'time', 'uploadDate',
  ];
  for (const key of keys) {
...
function stripJavascriptAssignment(raw: string): string {
...
function parseStructuredFile(raw: string): unknown {
  return JSON.parse(stripJavascriptAssignment(raw));
}
...
export async function parseHistoricalSocialArchive(file: File, platform: SocialPlatform): Promise<HistoricalImportResult> {
  ...
  } else if (extension.endsWith('.json') || extension.endsWith('.js')) {
    try {
      addParsedFile(parseStructuredFile(await file.text()), file.name);
    } catch {
      throw new Error('El archivo no tiene un formato JSON/JS válido de exportación.');
    }
  } else {
