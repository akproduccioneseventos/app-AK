...
export const AK_YOUTUBE_CHANNEL_URL =
  "https://www.youtube.com/@akproduccionesfiestasyeventos";
...
const CURATED_VIDEOS: Array<
  Pick<GaleriaVideo, "youtubeId" | "titulo" | "categoria" | "destacada">
> = [
  {
...
function inferCategory(title: string): string {
  ...
  if (normalized.includes("xv") || normalized.includes("15 ano"))
    return "XV Anos";
  if (normalized.includes("boda")) return "Bodas";
  if (normalized.includes("empresa") || normalized.includes("corporativo"))
    return "Empresariales";
  if (normalized.includes("club uruguay")) return "Club Uruguay";
  if (normalized.includes("cumple") || normalized.includes("1 ano"))
    return "Cumpleanos";
...
export function parseAkYoutubeFeed(xml: string): GaleriaVideo[] {
  ...
  return entries
    .map((entry, index) => {
      ...
      return toGaleriaVideo(
        videoId,
        title,
        entry.published || "",
        index + CURATED_VIDEOS.length,
      );
    })
